                        ...
                        <form action="{{ route('students.update', $student) }}" method="POST">
                                ...
                                <div class="input-style-1">
                                    ...
                                    <select name="class" id="class" class="form-control" required>
                                        ...
                                        @foreach($classLevels as $level)
                                            <option value="{{ $level }}" {{ old('class', $student->class) == $level ? 'selected' : '' }}>Kelas {{ $level }}</option>
                                        @endforeach
                                    </select>
                                    @error('class')
                                        <span class="text-danger small">{{ $message }}</span>
                                ...
                                <div class="input-style-1">
                                    ...
                                    <select name="major_id" id="major_id" class="form-control">
                                        ...
                                        @foreach($majors as $major)
                                            <option value="{{ $major->id }}" {{ old('major_id', $student->major_id) == $major->id ? 'selected' : '' }}>{{ $major->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('major_id')
                                        <span class="text-danger small">{{ $message }}</span>
                            ...
                            <div class="card-style mb-30">
                                <div class="input-style-1">
                                    ...
                                    <input type="date" name="enrollment_date" id="enrollment_date" value="{{ old('enrollment_date', $student->enrollment_date ? $student->enrollment_date->format('Y-m-d') : '') }}" required />
