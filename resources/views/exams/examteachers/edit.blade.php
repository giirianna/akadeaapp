...
@section('content')
                ...
                <div class="card-body">
                    ...
                    <div class="mt-3">
                        ...
                        <textarea name="description" class="form-control" rows="2"
                            placeholder="Tambahkan instruksi...">{{ old('description', $exam->description) }}</textarea>
                    </div>

                    <!-- File Upload untuk Import Soal -->
                    <div class="mt-4 p-3 border rounded-2" style="background-color: #f5f5f5;">
                        <label class="form-label fw-medium text-muted mb-2">Atau Import Soal dari File</label>
                        ...
                        <div class="input-group">
                            ...
                            <button class="btn btn-info" type="button" onclick="importQuestionsFromFile()">
                                <i class="lni lni-upload"></i> Import Soal
                            </button>
                        </div>
                        <small class="d-block text-muted mt-2">
                            <strong>Format File:</strong><br>
                            Kolom: Tipe | Pertanyaan | OpsiA | OpsiB | OpsiC | OpsiD | OpsiE | Jawaban<br>
                            Tipe: multiple_choice atau essay<br>
                            <a href="{{ route('exams.template.download') }}" target="_blank" class="text-info fw-bold">
                                <i class="lni lni-download"></i> Download Template Excel
                            </a>
                        </small>
                    </div>
                </div>
    ...
    <script>
        ...
        async function importQuestionsFromFile() {
            ...
            try {
                const response = await fetch('{{ route("exams.import-questions", $exam) }}', {
                    method: 'POST',
                    headers: {
                ...
                if (result.success) {
                    alert(`Berhasil mengimport ${result.count} soal!`);
